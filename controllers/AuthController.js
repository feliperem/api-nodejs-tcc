    ...
    register: async function(req, res) {
        try {
            ...
            if (!u) {
                ...
                res.status(403).json({message: 'Email registrado', error: {}});
            }
        }
        catch(e) {
            res.status(500).json({message: 'Erro ao salvar usuario', error: e});
        }
    },
        ...
        UserModel.findOne({email: email}).lean().exec(function(err, user) {
            if(err) {
                return res.status(500).json({
                    message: 'Erro no server', error: err });
            }
        ...
        jwt.verify(token, consts.keyJWT, 
            (err, decoded) => {
                ...
                UserModel.findById(id).lean().exec(function(err, user) {
                    if (err || !user) {
                        return res.status(500).json({
                            message: 'Erro ao retornar dados de usuario', error: err})
                    }
                    let token = jwt.sign({_id: user._id}, consts.keyJWT,{expiresIn: consts.expiresJWT});
                    delete user.password;                        
                    return res.json({...user, token: token});
